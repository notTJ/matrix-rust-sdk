...
mod test {
    ...
    fn deserialization() -> Result<(), serde_json::Error> {
        // `m.room_key` test
        let json = crate::types::events::room_key::test::json();
        let event: ToDeviceEvents = serde_json::from_value(json.clone())?;

        assert_matches!(event, ToDeviceEvents::RoomKey(_));
        let serialized = serde_json::to_value(event)?;
        assert_eq!(json, serialized);

        // `m.forwarded_room_key` test
        let json = forwarded_room_key_event();
        let event: ToDeviceEvents = serde_json::from_value(json.clone())?;

        assert_matches!(event, ToDeviceEvents::ForwardedRoomKey(_));
        let serialized = serde_json::to_value(event)?;
        assert_eq!(json, serialized);

        // `m.room_key_request` test
        let json = room_key_request_event();
        let event: ToDeviceEvents = serde_json::from_value(json.clone())?;

        assert_matches!(event, ToDeviceEvents::RoomKeyRequest(_));
        let serialized = serde_json::to_value(event)?;
        assert_eq!(json, serialized);

        // `m.secret.send` test
        let json = crate::types::events::secret_send::test::json();
        let event: ToDeviceEvents = serde_json::from_value(json.clone())?;

        assert_matches!(event, ToDeviceEvents::SecretSend(_));
        let serialized = serde_json::to_value(event)?;
        assert_eq!(json, serialized);

        // `m.secret.request` test
        let json = secret_request_event();
        let event: ToDeviceEvents = serde_json::from_value(json.clone())?;

        assert_matches!(event, ToDeviceEvents::SecretRequest(_));
        let serialized = serde_json::to_value(event)?;
        assert_eq!(json, serialized);

        // Unknown event test
        let json = custom_event();
        let event: ToDeviceEvents = serde_json::from_value(json.clone())?;

        assert_matches!(event, ToDeviceEvents::Custom(_));
        let serialized = serde_json::to_value(event)?;
        assert_eq!(json, serialized);

        // `m.key.verification.request` test
        let json = key_verification_event();
        let event: ToDeviceEvents = serde_json::from_value(json.clone())?;

        assert_matches!(event, ToDeviceEvents::KeyVerificationRequest(_));
        let serialized = serde_json::to_value(event)?;
        assert_eq!(json, serialized);

        // `m.dummy` test
        let json = dummy_event();
        let event: ToDeviceEvents = serde_json::from_value(json.clone())?;

        assert_matches!(event, ToDeviceEvents::Dummy(_));
        let serialized = serde_json::to_value(event)?;
        assert_eq!(json, serialized);

        // `m.room.encrypted` test
        let json = room_encrypted_event();
        let event: ToDeviceEvents = serde_json::from_value(json.clone())?;

        assert_matches!(event, ToDeviceEvents::RoomEncrypted(_));
        let serialized = serde_json::to_value(event)?;
        assert_eq!(json, serialized);
